...
# Menambahkan background CSS pada tampilan
st.markdown(
    '''<style>
    body, .stApp {
    ...
    h1, .stApp h1, .stApp h2, .stApp h3, .stApp h4, .stApp h5, .stApp h6 {
        color: #f5f6fa !important;
...
st.title("💬 SiCakap (Sistem Chat Pintar)")
st.caption("Mau curhat, nanya, atau cari solusi? Aku selalu siap jawab dengan ramah dan cepat.")
...
with st.sidebar:
    ...
    # Fitur upload file
    uploaded_file = st.file_uploader("Upload File Disini (PDF/TXT/Excel)", type=["pdf", "txt", "xlsx", "csv"])
...
if not google_api_key:
    st.info("Mohon inputkan Google AI API key yang kamu miliki pada sidebar agar kita mulai ngobrol ya!", icon="🗝️")
    st.stop()
...
if 'uploaded_file' in locals() and uploaded_file is not None:
    if uploaded_file.type == "application/pdf":
        try:
            ...
            file_text = f"[Gagal membaca PDF: {e}]"
    elif uploaded_file.type == "text/plain":
        file_text = uploaded_file.read().decode("utf-8")
    elif uploaded_file.type in ["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "text/csv"] or uploaded_file.name.endswith(('.xlsx', '.csv')):
        try:
            import pandas as pd
            ...
            excel_summary = f"Ringkasan data Excel:\n- Jumlah baris: {df.shape[0]}\n- Jumlah kolom: {df.shape[1]}\n- Nama kolom: {', '.join(df.columns.astype(str))}\n- 5 data teratas:\n{df.head().to_string(index=False)}"
        except Exception as e:
            excel_summary = f"[Gagal membaca file Excel: {e}]"

# Check if the user has entered a message.
if prompt:
    # Gabungkan prompt dengan isi file jika ada
    prompt_with_file = prompt
    if file_text:
        prompt_with_file += f"\n\nBerikut isi file yang diupload:\n{file_text}"
    if excel_summary:
        prompt_with_file += f"\n\n{excel_summary}"
    ...
    try:
        ...
        for msg in st.session_state.messages:
            if msg["role"] == "user":
                # Jika ini adalah pesan terakhir dari user, gunakan prompt_with_file
                if msg == st.session_state.messages[-1]:
                    messages.append(HumanMessage(content=prompt_with_file))
                else:
